...
// to place everything from offset 0x1000, regardless that the actual mbr is 0x200.
// note: the official uboot project use 0x200 here as MBR_SIZE
...
#define DCD_COMMAND_HEADER(tag, len, params) ((tag) | (cpu_to_be16((len)) << 8) | (params) << 24)
#define DCD_WRITE_TAG (0xcc)
#define DCD_CHECK_TAG (0xcf)
#define DCD_NOP_TAG (0xc0)

#define DCD_PARAMS(SET, MASK, LEN) (((LEN)&0x7) | ((SET) ? (1 << 4) : 0x0) | ((MASK) ? (1 << 3) : 0x0))

#define DCD_WRITE_HEADER(n, params) DCD_COMMAND_HEADER(DCD_WRITE_TAG, 4 + (n)*8, (params))
#define DCD_CHECK_HEADER(params) DCD_COMMAND_HEADER(DCD_CHECK_TAG, 16, (params))
#define DCD_CHECK_HEADER_NO_COUNT(params) DCD_COMMAND_HEADER(DCD_CHECK_TAG, 12, (params))
...
enum boot_type { QSPI_BOOT, SD_BOOT, EMMC_BOOT, INVALID_BOOT };
...
enum data_file_cmd { SET_SIZE, INVALID_DATA_FILE_CMD };
...
static struct reserved_range reserved_sram[] = {
	{ .start = (void *)0x34002000, .end = (void *)0x34002fff },
	{ .start = (void *)0x34008000, .end = (void *)0x34079c00 },
};
...
static void enforce_reserved_ranges(void *image_start, int image_length)
{
	...
	for (i = 0; i < ARRAY_SIZE(reserved_sram); i++)
		if (image_start < reserved_sram[i].end && image_end > reserved_sram[i].start) {
			fprintf(stderr, "Loading data of size 0x%x at %p forbidden.", image_length, image_start);
			fprintf(stderr, " Range %p --- %p is reserved!\n", reserved_sram[i].start,
				reserved_sram[i].end);
			exit(EXIT_FAILURE);
		}
}
...
static bool is_in_overlap(struct image_comp *comp1, struct image_comp *comp2)
{
...
static void s32_compute_dyn_offsets(struct image_comp **parts, size_t n_parts)
{
	...
	while ((start_index < n_parts) && (parts[start_index]->offset != AUTO_OFFSET))
		start_index++;
	...
	for (aindex = start_index; aindex < n_parts; aindex++) {
		...
		if (parts[aindex]->offset != AUTO_OFFSET) {
			...
			qsort(&parts[0], aindex + 1, sizeof(parts[0]), image_parts_comp);
			continue;
		}
...
{
	memcpy(qspi_params, iconfig.qspi_params.data, iconfig.qspi_params.size);
}
...
void s32cc_set_header(void *header, struct stat *sbuf, int unused, struct image_tool_params *tool_params)
{
	...
	if (dcd_data) {
		...
		*(uint16_t *)(dcd + DCD_HEADER_LENGTH_OFFSET) = cpu_to_be16(dcd_data_size);
	}
	...
	if (tool_params->ep < tool_params->addr) {
		fprintf(stderr, "The entrypoint is higher than the load address (0x%x < 0x%x)\n", tool_params->ep,
			tool_params->addr);
		exit(1);
	}
	...
	if (!iconfig.flash_boot) {
		enforce_reserved_ranges((void *)(uintptr_t)app_code->ram_start_pointer, app_code->code_length);
	} else {
	...
	image_layout.code.size = sbuf->st_size - image_layout.app_code.offset - image_layout.app_code.size;
...
static struct image_comp *get_image_qspi_params(struct program_image *program_image)
{
	if (iconfig.flash_boot)
...
static struct image_comp *get_image_hse_fw(struct program_image *program_image)
{
	if (iconfig.hse.enable)
...
static struct image_comp *get_image_hse_sys_img(struct program_image *program_image)
{
	if (iconfig.hse.enable)
...
static struct image_comp *get_image_mbr(struct program_image *program_image)
{
...
static struct image_comp *get_image_dcd(struct program_image *program_image)
{
	if (!iconfig.dcd.size)
		return NULL;
...
static void set_headers_size(struct program_image *program_image)
{
	...
	if (program_image->dcd.size > DCD_MAXIMUM_SIZE) {
		fprintf(stderr, "DCD area exceeds the maximum allowed size (0x%x\n)", DCD_MAXIMUM_SIZE);
		exit(EXIT_FAILURE);
	}

	program_image->hse_fw.size = iconfig.hse.fw_size;
	if (program_image->hse_fw.size > HSE_FW_MAX_SIZE) {
		fprintf(stderr, "HSE FW area exceeds the maximum allowed size (0x%lx\n)", HSE_FW_MAX_SIZE);
		exit(EXIT_FAILURE);
	}
}
...
static int s32g2xx_build_layout(struct program_image *program_image, size_t *header_size, void **image)
{
	...
	*header_size = program_image->app_code.offset + program_image->app_code.size;
...
static struct ivt *get_ivt_from_raw_blob(const unsigned char *ptr, int size, bool *qspi_ivt)
{
	int offset, min_size;
...
static void print_layout(struct layout_comp **comps, size_t n)
{
	...
	fprintf(stderr, "  Image Layout\n");
	for (i = 0u; i < n; i++) {
		if (!comps[i]->offset && !comps[i]->size)
			continue;

		fprintf(stdout, "      %sOffset: 0x%x\t\t", comps[i]->line_desc, comps[i]->offset);

		if (comps[i]->size)
			fprintf(stdout, "Size: 0x%x", comps[i]->size);
...
void s32cc_print_header(const void *data)
{
	...
	struct layout_comp ivt_comp, qspi, dcd, hse_fw, hse_img, app_hdr, app_comp;
	struct layout_comp *comps[] = { &ivt_comp, &qspi, &dcd, &hse_fw, &hse_img, &app_hdr, &app_comp };
	...
	if (ivt->dcd_pointer) {
		dcd_len = (uint16_t *)(data8 + ivt->dcd_pointer + DCD_HEADER_LENGTH_OFFSET);
...
static int parse_hse_cmd(char *line)
{
	...
	ret = map_file(path, &iconfig.hse.fw_data, &iconfig.hse.fw_size);
free_mem:
	free(path);
	return ret;
}
...
static int get_dcd_cmd_args(char *line, struct dcd_args *dcd)
{
	 ...
	 */
	ret = sscanf(line, "%[^ \t]%*[^0]0x%" PRIx32 "%*[^0]0x%" PRIx32 "%*[^0]0x%" PRIx32 "%*[^0]0x%" PRIx32, cmd,
		     &dcd->nbytes, &dcd->addr, &dcd->value, &dcd->count);
...
static int push_to_dcd_array(uint32_t data)
{
	...
	if (iconfig.dcd.allocated_size <= iconfig.dcd.size + elem_size) {
		...
		if (!iconfig.dcd.data) {
			// free(backup);
			fprintf(stderr, "Failed to allocate %zu bytes for DCD array\n", iconfig.dcd.allocated_size);
...
static int parse_qspi_cmd(char *line)
{
	...
	ret = map_file(path, &iconfig.qspi_params.data, &iconfig.qspi_params.size);
	if (ret)
		return ret;
...
int s32cc_check_params(struct image_tool_params *params)
{
	...
	if (!params->lflag && !strlen(params->imagename)) {
		fprintf(stderr,
	 ...
	 */
		return (params->dflag && (params->fflag || params->lflag)) ||
		       ...
		       (params->lflag && (params->dflag || params->fflag)) || (params->xflag) ||
...
struct image_type_params s32cc_image_type_params = { .name = "NXP S32 Common Chassis Boot Image",
						     .header_size = 0,
						     .hdr = NULL,
						     .check_params = s32cc_check_params,
						     .verify_header = s32cc_verify_header,
						     .print_header = s32cc_print_header,
						     .set_header = s32cc_set_header,
						     .extract_subimage = NULL,
						     .check_image_type = s32cc_check_image_type,
						     .fflag_handle = NULL,
						     .vrec_header = s32cc_vrec_header };

struct image_type_params *s32cc_image_type_params_ptr = &s32cc_image_type_params;
